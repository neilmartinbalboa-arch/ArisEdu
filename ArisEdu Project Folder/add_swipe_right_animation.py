...
def process_file(file_path):
    try:
        ...
        # Pattern matches the existing prevFlashcardBtn click handler
        old_code_pattern = re.compile(
            r"prevFlashcardBtn\.addEventListener\('click', \(\) => \{\s+"
            r"currentFlashcard = \(currentFlashcard - 1 \+ flashcardOrder\.length\) % flashcardOrder\.length;\s+"
            r"updateFlashcard\(\);\s+"
            r"\}\);",
            re.DOTALL
        )
        ...
        prevFlashcardBtn.addEventListener('click', () => {
          ...
          // Swipe Right Animation (Exit to Right)
        ...
        if old_code_pattern.search(content):
            ...
            changed = True
            print(f"Added swipe right animation to {os.path.basename(file_path)}")
...
def main():
    ...
    print("Swipe right animation update complete.")
